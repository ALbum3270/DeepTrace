...
from .editor import ChiefEditor
...
def writer_node(state: SwarmState) -> Dict[str, Any]:
    ...
    logger.info(
        f"--- Writer Node: Drafting Section {idx + 1}/{len(outline.sections)}: {section.title} ---"
    )
...
def critic_node(state: SwarmState) -> Dict[str, Any]:
    ...
    if not outline:
        return {"critique_feedback": None}  # Should not happen in normal flow
    ...
    logger.info(
        f"Critic Result: Score={result.score}, NLI={result.nli_state}, R={result.revision_needed}"
    )
...
def should_continue(
    state: SwarmState,
) -> Literal["writer_node", "next_section", "auditor"]:
    """
    Edge Logic: Router
    ...
    if feedback:
        if rev_count < MAX_REVISION_LOOPS_PER_SECTION:
            ...
            logger.warning(
                f"Routing: MAX REVISIONS ({MAX_REVISION_LOOPS_PER_SECTION}) reached. Forcing progress."
            )
    ...
    logger.info("Routing: All Sections Done -> Audit")
    return "auditor"
...
def auditor_node(state: SwarmState) -> Dict[str, Any]:
    """
    Node: Consistency Auditor
    Action: Check for cross-section conflicts
    """
    logger.info("--- Auditor Node: Consistency Check ---")
    ...
    for section in outline.sections:
        current = drafts.get(section.id, "")
        # Check consistency
        res = auditor.check_consistency(current, previous_drafts)
        audit_results[section.id] = res

        # Add to history for next check
        previous_drafts.append(current)
...
def finalize_node(state: SwarmState) -> Dict[str, Any]:
    """
    Node: Editor (Final Assembly)
    Action: Assemble report and handle conflicts
    """
    logger.info("--- Editor Node: Assembling Report ---")
    outline = state["outline"]
    drafts = state["draft_sections"]
    ...
    full_report = editor.assemble_report(outline, drafts, audit_results)
...
def create_swarm_graph():
    ...
    builder.add_edge("auditor", "finalizer")
