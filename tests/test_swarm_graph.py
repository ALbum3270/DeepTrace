...
from src.agents.swarm.graph import (
    ...
    critic_node,
    should_continue,
    MAX_REVISION_LOOPS_PER_SECTION,
    create_swarm_graph,
)
...
def test_router_logic_finish():
    ...
    # So it should go to auditor (then finalize)

    next_node = should_continue(state)
    assert next_node == "auditor"
...
@pytest.mark.asyncio
async def test_full_graph_flow_with_retry(
    mock_editor, mock_auditor, mock_critic, mock_writer, mock_director
):
    """
    ...
    # Critic: Fail then Pass
    mock_critic.verify_section.side_effect = [
        CritiqueResult(
            score=4.0, feedback="Fix it", nli_state="CONTRADICT", revision_needed=True
        ),
        CritiqueResult(
            score=9.0, feedback="", nli_state="ENTAIL", revision_needed=False
        ),
    ...
    # Auditor (Mock behavior within node)
    # The auditor_node calls auditor.check_consistency.
    mock_auditor.check_consistency.return_value = MagicMock(
        passed=True, conflict_detected=False
    )

    # Editor
    mock_editor.assemble_report.return_value = "Final Report V2"
    ...
    assert final_state["final_report"] == "Final Report V2"

    # Verify Calls
    assert mock_writer.write_section.call_count == 2
    assert mock_critic.verify_section.call_count == 2
    assert mock_auditor.check_consistency.call_count == 1  # One section
    assert mock_editor.assemble_report.call_count == 1
